...
describe("Deployment happy path", async () => {
  ...
  it ("processTestSuite", async () => {
    ...
    chai.expect(fsAccessStubs.callCount).eq(9)
    // check for presense of workspace directory on the disk
    ...
    // debugging tools
    // for (let i = 12; i < execStub.callCount; i++) {
    ...
    // chai.expect(execStub.getCall(5).args[1]).eq(undefined)
